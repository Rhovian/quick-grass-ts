import { THREE, RenderPass, ShaderPass, FXAAShader, ACESFilmicToneMappingShader } from './three-defs';
...
const GammaCorrectionShader2: ShaderType = {
...
export class ThreeJSController extends entity.Component {
  ...
  #threejs_: THREE.WebGLRenderer | null;
  #csm_: FakeCSM | null;

  #ssaoPass_: any | null;
  #opaqueScene_: THREE.Scene | null;
  #opaquePass_: RenderPass | null;
  #waterScene_: THREE.Scene | null;
  #waterPass_: RenderPass | null;

  #opaqueCamera_: THREE.PerspectiveCamera | null;
  #waterCamera_: THREE.PerspectiveCamera | null;

  #transparentScene_: THREE.Scene | null;
  #transparentPass_: RenderPass | null;
  #transparentCamera_: THREE.PerspectiveCamera | null;

  #waterTexturePass_: ShaderPass | null;

  #fxaaPass_: ShaderPass | null;
  #acesPass_: ShaderPass | null;
  #gammaPass_: ShaderPass | null;
  #copyPass_: ShaderPass | null;

  grassTimingAvg_: number;
  timerQuery: any;
  sun_: THREE.DirectionalLight;
  waterBuffer_: THREE.WebGLRenderTarget;
  readBuffer_: THREE.WebGLRenderTarget;
  writeBuffer_: THREE.WebGLRenderTarget;
  ...
  constructor() {
    ...
    this.#transparentScene_ = null;
    this.#transparentCamera_ = null;
    this.#transparentPass_ = null;
  ...
  InitEntity(): void {
    ...
    const waterParams: THREE.WebGLRenderTargetOptions = {
    ...
    const bufferParams: THREE.WebGLRenderTargetOptions = {
      type: THREE.HalfFloatType,
  ...
  get WaterDepthTexture(): THREE.Texture {
    return this.waterBuffer_.texture;
  }
  ...
  onWindowResize_(): void {
    if (!this.#threejs_ || !this.#opaqueCamera_ || !this.#waterCamera_ || 
        !this.#transparentCamera_ || !this.#waterTexturePass_ || 
        !this.#fxaaPass_ || !this.#csm_) return;
  ...
  AddSceneObject(obj: THREE.Object3D, params?: { pass?: string }): void {
    params = params || {};

    if (params.pass === 'water' && this.#waterScene_) {
      this.#waterScene_.add(obj);
    } else if (params.pass === 'transparent' && this.#transparentScene_) {
      this.#transparentScene_.add(obj);
    } else if (this.#opaqueScene_) {
      this.#opaqueScene_.add(obj);
    }
  }

  Render(timeElapsedS: number): void {
    if (!this.#threejs_ || !this.#opaqueCamera_ || !this.#waterCamera_ || 
        !this.#transparentCamera_ || !this.#waterPass_ || !this.#opaquePass_ || 
        !this.#transparentPass_ || !this.#waterTexturePass_ || !this.#gammaPass_) return;
    ...
    (this.writeBuffer_ as any).ACTIVE_HAS_OPAQUE = true; 
    (this.readBuffer_ as any).ACTIVE_HAS_OPAQUE = false; 
    this.swapBuffers_();
    ...
    this.#waterPass_.render(this.#threejs_, null, this.writeBuffer_, timeElapsedS, false);
    ...
    (this.writeBuffer_ as any).ACTIVE_HAS_WATER = true;
    (this.readBuffer_ as any).ACTIVE_HAS_WATER = false;
    this.swapBuffers_();
